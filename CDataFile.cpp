...
// well in a linux environment (with some mTay have
...
CDataFile::CDataFile(t_Str szFileName, int mode)
{
...
bool CDataFile::Load(t_Str szFileName)
{
	...
	if ( File.is_open() )
	{
		...
		while ( !bDone )
		{
			std::memset(buffer, 0, (std::size_t)MAX_BUFFER_LEN);
			...
			if ( szLine.find_first_of('[') == 0 ) // new section
			{
				...
				if ( szLine.find("color_surf_range",0) == 0 )
				{
					iRetour = 1;
				}
				if ( szLine.find("names",0) == 0 )
				{
					iRetour = 1;
				}

				//if ( szLine.find_first_of('Dimensions') == 0 ){iRetour = 1;}
				//if ( szLine.find_first_of('landmarks') == 0 ){iRetour = 1;}
				//if ( szLine.find_first_of('individuals') == 0 ){iRetour = 1;}
				//if ( szLine.find_first_of('polygons') == 0 ){iRetour = 1;}
				//if ( szLine.find_first_of('wireframe') == 0 ){iRetour = 1;}
...
float CDataFile::GetFloat(t_Str szKey, t_Str szSection)
{
	...
	return (float)std::atof( szValue.c_str() );
}
...
#ifdef WIN32
	OutputDebugString((LPCSTR)szMsg.c_str());
#endif
