...
//vtkCxxRevisionMacro(LandmarksTransform, "$Revision: 1.1 $");
vtkStandardNewMacro(LandmarksTransform);

//----------------------------------------------------------------------------
LandmarksTransform::LandmarksTransform()
{
	this->Mode = VTK_LANDMARK_SIMILARITY;
	this->SourceLandmarks = NULL;
	this->TargetLandmarks = NULL;
	this->LandmarksWeight = NULL;
}
...
LandmarksTransform::~LandmarksTransform()
{
	...
	if (this->LandmarksWeight){
		this->LandmarksWeight->Delete();
	}
}

//----------------------------------------------------------------------------
void LandmarksTransform::PrintSelf(ostream& os, vtkIndent indent)
{
...
void LandmarksTransform::InternalUpdate()
{
	vtkIdType i;
	int j;
	...
	*/
	...
	for (pt = 0; pt<N_PTS; pt++)
	{
		...
		double weight;
		if (LandmarksWeight != NULL){
			weight = LandmarksWeight->GetValue(pt);
		}
		else{
			weight = 1.;
		}
		for (i = 0; i<3; i++)
		{
			M[i][0] += weight*a[i] * b[0];
			M[i][1] += weight*a[i] * b[1];
			M[i][2] += weight*a[i] * b[2];
			...
			{
				AAT[i][0] += weight*a[i] * a[0];
				AAT[i][1] += weight*a[i] * a[1];
				AAT[i][2] += weight*a[i] * a[2];
			}
		}
...
//------------------------------------------------------------------------
void LandmarksTransform::SetTargetLandmarks(vtkPoints *target)
{
...
void LandmarksTransform::SetLandmarksWeight(vtkFloatArray *weight){

	if (weight == NULL)
		return;

	LandmarksWeight = vtkFloatArray::New();
	this->LandmarksWeight->DeepCopy(weight);
...
//----------------------------------------------------------------------------
void LandmarksTransform::Inverse()
{
...
//----------------------------------------------------------------------------
vtkAbstractTransform *LandmarksTransform::MakeTransform()
{
	return LandmarksTransform::New();
}

//----------------------------------------------------------------------------
void LandmarksTransform::InternalDeepCopy(vtkAbstractTransform *transform)
{
	LandmarksTransform *t = (LandmarksTransform *)transform;
...
const char * LandmarksTransform::GetModeAsString() // attention 'inline' enlever
{
	switch (this->Mode)
	{
