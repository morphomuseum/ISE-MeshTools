...
typedef pointNdimAxe* point;

////////////////////////////////////////////////   KDTREE  //////////////////////////////////////////////////////

//construit le KDtree
void KDtree::Build(const  vtkSmartPointer<vtkFloatArray> list, bool bool_color){
	...
	cout << "KDTREE (en cours) ::  ";
	root = new Node(ptlist, indCollist, 0,nbComponents);// construction des noeuds
...
vtkSmartPointer<vtkFloatArray> KDtree::Closest_to_pt(const double *point, float *maxdist2, vtkSmartPointer<vtkFloatArray> tab_weight, double alpha[8])
{
	float mindist;
...
vtkFloatArray* KDtree::PointsInRadius(double *point, float R){
...
//Constructeur
Node::Node(vtkSmartPointer<vtkFloatArray> ptlist, vtkSmartPointer<vtkFloatArray> indCollist, int depth, int nbComponents){
	...
	if (n <= v) {
		...
		//initialisation min et max pour chaque coordonnée
		for (int j = 0; j < d; j++){
		...
		//Sorted(structure.N.splitaxis, list, infolist);
		Sort(list, infolist, structure.N.splitaxis);
		...
		structure.N.left_child = new Node(left, infoleft, depth + 1, nbComponents);
		structure.N.right_child = new Node(right, inforight, depth + 1,nbComponents);
	}
}
...
void Node::Candidat(double* point, double* infopoint, vtkSmartPointer<vtkFloatArray> candidatPoint){
	int d = nbComponents;
...
void Node::find_closest_to_pt(const double *point, int depth, float *mindist, vtkSmartPointer<vtkFloatArray> candidatPoint, vtkSmartPointer<vtkFloatArray> tab_weight, double alpha[8])
{
...
void Node::PointsInRadius(double *point, int depth, float R, vtkFloatArray *KcandidatPoint, vtkSmartPointer<vtkFloatArray> tab_weight, double alpha[8]){
...
void Sort(vtkSmartPointer<vtkFloatArray> list, vtkSmartPointer<vtkFloatArray> infolist, int axe){
	int N = list->GetNumberOfTuples();
	pointNdimAxe *listPaxe = (pointNdimAxe*)malloc(N*sizeof(pointNdimAxe));
	...
	qsort(listPaxe, N, sizeof(pointNdimAxe), CompareAxe);
...
void Sorted(int firstAxe, vtkFloatArray* ptlist, vtkSmartPointer<vtkFloatArray> infolist){
	...
	list->SetNumberOfComponents(d);
	list->SetNumberOfTuples(n);
	Sort(ptlist, infolist, firstAxe);// premier trie
	list->DeepCopy(ptlist);
		...
		if (!Appartient(axe, listAxe)){
			for (int i = 0; i < n; i++){
				if (i + 1 < n) {
					...
					if (IsEqualAccordingToAxe(point1, point2, d, listAxe)){
						...
						if (listTemp->GetNumberOfTuples() != 0){
							...
							//trie
							Sort(listTemp, infolistTemp, axe);
...
bool Appartient(int axe, vtkFloatArray* listAxe){ 
