...
#ifndef __LandmarkTransform_h
#define __LandmarkTransform_h
...
class VTKCOMMONTRANSFORMS_EXPORT LandmarkTransform : public vtkLinearTransform
{
public:
  static LandmarkTransform *New();

  vtkTypeMacro(LandmarkTransform,vtkLinearTransform);
...
protected:
  ...
  int Mode;
private:
  LandmarkTransform(const LandmarkTransform&);  // Not implemented.
  void operator=(const LandmarkTransform&);  // Not implemented.
};

//BTX
inline const char *LandmarkTransform::GetModeAsString()
{
  switch (this->Mode)
    {
